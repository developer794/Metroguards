...
// Metro Guards Official Rates (as of 2025)
 ...
 */
export function getApplicableRate(date, hour = 9) {
  ...
  switch (dayType) {
    case 'new_years_eve':
      return {
        ...
        rate: SERVICE_RATES.new_years_eve.rate,
        name: SERVICE_RATES.new_years_eve.name,
      };
    case 'public_holiday':
      return {
        rateType: 'public_holiday',
        rate: SERVICE_RATES.public_holiday.rate,
        name: SERVICE_RATES.public_holiday.name,
      };
    case 'sunday':
      return {
        rateType: 'sunday',
        rate: SERVICE_RATES.sunday.rate,
        name: SERVICE_RATES.sunday.name,
      };
    case 'saturday':
      return {
        rateType: 'saturday',
        rate: SERVICE_RATES.saturday.rate,
        name: SERVICE_RATES.saturday.name,
      };
    default: // weekday
      if (hour >= 6 && hour < 18) {
        return {
          rateType: 'weekday_day',
          rate: SERVICE_RATES.weekday_day.rate,
          name: SERVICE_RATES.weekday_day.name,
        };
      }
      return {
        rateType: 'weekday_night',
        rate: SERVICE_RATES.weekday_night.rate,
        name: SERVICE_RATES.weekday_night.name,
      };
  }
}
 ...
 */
export function calculateShiftCost({ date, startHour = 9, hours = 8, guards = 1 }) {
  const rateInfo = getApplicableRate(date, startHour);
...
export function calculateQuickEstimate({
  ...
  switch (shiftType) {
    case 'night':
      avgRate = SERVICE_RATES.weekday_night.rate;
      break;
    case 'mixed':
      avgRate = (SERVICE_RATES.weekday_day.rate + SERVICE_RATES.weekday_night.rate) / 2;
      break;
    default:
      avgRate = SERVICE_RATES.weekday_day.rate;
  }
